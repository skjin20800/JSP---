...
public class Dispatcher2 implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		for (Method method : methods) {
			...
			if (requestMapping.value().equals(endPoint)) {
				isMatching = true;
				try {
					// 해당 메서드의 파라미터(클래스)를 파라미터에 담는다 
					Parameter[] params = method.getParameters(); // LoginDto
					String path = null;
					if (params.length != 0) {						
//						params[0].getType(); // 해당 파라미터의 클래스를 보여준다
						Object dtoInstance = params[0].getType().newInstance();// 담긴 파라미터 클래스의 객체를 생성해서 dtoInstance에 담는다
